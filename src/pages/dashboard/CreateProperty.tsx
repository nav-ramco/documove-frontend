  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <h1 className="text-xl font-bold text-gray-900 mb-1">List New Property</h1>
        <p className="text-gray-500 text-sm mb-6">Add property details and invite the seller. You can invite a buyer later once an offer is accepted.</p>
          ...
          {[1, 2].map(s => (
            <div key={s} className="flex-1">
              ...
              <p className={"text-xs mt-1 " + (step >= s ? 'text-accent font-medium' : 'text-gray-400')}>
                {s === 1 ? 'Property Details' : 'Invite Seller'}
              </p>
            </div>
          ))}
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button type="submit" disabled={loading} className="flex-1 bg-accent text-white py-2.5 rounded-lg font-medium hover:bg-accent-dark transition-colors disabled:opacity-50">
                  {loading ? 'Creating...' : 'List Property & Invite Seller'}
                </button>
              </div>
            </div>
          )}
